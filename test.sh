...
CALL='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"web_parse","arguments":{"url":"https://example.com","selector":"h1","method":"text"}}}'

RESPONSES=$(printf '%s\n%s\n%s\n%s\n' "$INIT" "$INITIALIZED" "$LIST" "$CALL" | node dist/index.js 2>/dev/null)
